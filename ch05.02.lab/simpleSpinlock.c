...
atomic_long bankAccount[100000000] = {0};
atomic_int lock_ary[100000000] = {0};
long getlock = 0;
long kpi[1000];
...
void simple_spinlock_lock(atomic_int* lock,int name) {
    int isunlock=0;
    //printf("number:%d\n",name);
    ...
    while (*lock == 0  &&  atomic_compare_exchange_weak(lock, &isunlock, name) == false) {
        isunlock=0;
    }
}

//解除鎖定
void simple_spinlock_unlock(atomic_int* lock,int name) {
    atomic_store(lock, name);
}
...
void moneyTransfer(void *givenName) {
	...
	//int min = pri();
	//printf("%d\n",min);
        simple_spinlock_lock(&(lock_ary[source]),name);
        simple_spinlock_lock(&(lock_ary[dest]),name);
        idx = source;
        //底下程式碼從source轉amount的錢到dest
        bankAccount[source]-=amount;
        bankAccount[dest]+=amount;
        //解鎖
        simple_spinlock_unlock(&(lock_ary[source]),name);
        simple_spinlock_unlock(&(lock_ary[dest]),name);
...
int main(int argc, char **argv) {
    ...
    numCPU = sysconf( _SC_NPROCESSORS_ONLN )-1;
   ...
   printAccountSummary();
    ...
    printAccountSummary();
    long kpi_sum=0;
	printf(YELLOW"manager 0, get lock:%ld個\n", kpi[0]);
    for(int i=1; i<=numCPU; i++)
        {	
		printf("%d, %ld\n",i,kpi[i]);
		kpi_sum+=kpi[i];
	}	
    printf(YELLOW"行員共有%d個\n", numCPU);
